...
static void bfs(const RaceState& rs, const Course& course)
{
  bfsed.clear();
  queue<Point> queue;
  const int ymax = course.length + course.vision;
  ...
  while (queue.size()) {
    ...
    for (const auto& d : ofs) {
      ...
      int ny = d.second + p.y;
      Point np(nx, ny);
      if (ny > course.length + course.vision || course.obstacle[ny][nx] == ObstState::OBSTACLE) {
        continue;
      }
      if (bfsed.count(np)) {
        continue;
  ...
  do {
	...
	if ((c->step != 0 ||
	  ...
	  if (nextPos.y >= course.length) {
	    if (bfsed[nextPos] < bfsed[best->state.position]) {
	      best = nextCand;
	    }
